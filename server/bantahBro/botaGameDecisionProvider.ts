...
export class GameSdkDecisionProvider implements BotaAgentDecisionProvider {
  ...
  private fallback = new MockGameDecisionProvider();
  ...
  async decide(input: {
    ...
    try {
      ...
      return {
        ...fallback,
        rationale:
          `GAME SDK unavailable; mock fallback used. ` +
...
export function getBotaDecisionProvider(): BotaAgentDecisionProvider {
  const provider = String(process.env.BOTA_GAME_PROVIDER || "").trim().toLowerCase();
