...
export default function History() {
  ...
  const getChallengeFallbackImage = () => "/assets/bantahblue.svg";
  ...
  const isRealChallengeCover = (url?: string | null) => {
    const normalized = normalizeMediaUrl(url);
    if (!normalized || normalized === "TEST IMAGE") return false;

    const lowered = normalized.toLowerCase();

    // Ignore known placeholders / mock assets
    if (
      lowered.includes("/assets/versus.svg") ||
      lowered.includes("/assets/default-event-banner.jpg")
    ) {
      return false;
    }
    return true;
  ...
  const getActivityImage = (activity: any) => {
    if (activity.type === "event") {
      const eventImage =
        ...
        activity.image_url ||
        activity.banner_url;
      const normalizedEventImage = normalizeMediaUrl(eventImage);
      if (normalizedEventImage && normalizedEventImage !== "TEST IMAGE") {
        return normalizedEventImage;
      }
      return "/assets/default-event-banner.jpg";
    }
    ...
    if (isRealChallengeCover(challengeCover)) {
      return normalizeMediaUrl(challengeCover)!;
    }

    return getChallengeFallbackImage();
  };
  ...
  const canEditActivity = (activity: any) => {
    ...
    if (activity.type === "event") {
      ...
      const now = new Date();
      const isCreator = activity.creatorId === user.id || activity.createdBy === user.id;
      return isCreator && now < endDate;
    } else if (activity.type === "challenge") {
      const dueDate = new Date(activity.dueDate);
      const now = new Date();
      const status = String(activity.status || "").toLowerCase();
      const isEditableStatus = status === "pending" || status === "open";
      const isChallenger =
        activity.challenger === user.id ||
        activity.challengerId === user.id ||
        activity.challengerUser?.id === user.id;
      return isChallenger && now < dueDate && isEditableStatus;
    }
    return false;
  };
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src =
                    activity.type === "challenge"
                      ? getChallengeFallbackImage()
                      : "/assets/default-event-banner.jpg";
                }}
              />
            </div>
          ...
          <div className="ml-2 flex space-x-1 flex-shrink-0">
            {canEditActivity(activity) && (
              <Button
                ...
                className="h-7 px-2 text-xs border-0 bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-100 dark:hover:bg-slate-600"
              >
            ...
            <Button
              ...
              onClick={() => {
                if (activity.type === "challenge") {
                  navigate(`/challenges/${activity.id}/chat`);
                } else if (activity.type === "event") {
                  navigate(`/events/${activity.id}/chat`);
                }
              }}
            >
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 theme-transition pb-[50px]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <Tabs defaultValue="created" className="space-y-2">
              <TabsList className="grid w-full grid-cols-4 h-8 sm:h-9 border-0 shadow-none bg-transparent gap-1">
              ...
              {eventsLoading || challengesLoading ? (
                ...
                <Card className="bg-white dark:bg-slate-800 mobile-compact-card border-0 md:border md:border-slate-200 md:dark:border-slate-700">
                  <CardContent className="text-center py-6">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400">
                      Join or accept challenges to see them here.
                    </p>
                  </CardContent>
                </Card>
              ) : (
