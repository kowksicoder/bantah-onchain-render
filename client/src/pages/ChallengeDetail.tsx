            ...
            <div className="flex items-center gap-8 flex-1">
              ...
              <div className="flex flex-col items-center gap-1 text-center min-w-[80px]">
                <UserAvatar
                  ...
                  profileImageUrl={match.user?.profileImageUrl}
                  size={40}
              ...
              <div className="flex flex-col items-center gap-1 text-center min-w-[80px]">
                {match.matchedWithUser ? (
                  <>
                    <UserAvatar
                      userId={match.matchedWithUser?.id}
                      username={match.matchedWithUser?.username}
                      firstName={match.matchedWithUser?.firstName}
                      profileImageUrl={match.matchedWithUser?.profileImageUrl || match.matchedWithUser?.profile_image_url}
                      size={40}
                      className="w-10 h-10 ring-2 ring-primary/10"
                    />
                    <span className="text-xs font-medium text-gray-600 dark:text-gray-400">
                      {getUserDisplayName({
                        id: match.matchedWithUser?.id,
                        firstName: match.matchedWithUser?.firstName,
                        username: match.matchedWithUser?.username,
                      }, "opponent")}
                    </span>
...
export default function ChallengeDetail() {
  ...
  const isAdminOpenChallenge = challenge?.adminCreated && challenge?.status === 'open';
  const canJoinQueue = isAdminOpenChallenge && user && !isParticipant;

  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 pb-24">
      <DynamicMetaTags challenge={challenge} pageType="challenge" />
