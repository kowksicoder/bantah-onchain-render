...
const userActionSchema = z.object({
  action: z.enum(['ban', 'unban', 'balance', 'admin', 'message', 'password']),
...
export default function AdminUsersManagement() {
  ...
  const { data: users = [], isLoading, refetch } = useAdminQuery("/api/admin/users", {
    retry: false,
  });
  ...
  const handlePromoteToAdmin = (user: User) => {
    const label = user.firstName || user.username || user.id;
    const confirmed = confirm(`Promote ${label} to admin?`);
    if (!confirmed) return;
    ...
    userActionMutation.mutate({
      ...
      action: "admin",
      value: "true",
      reason: "Promoted to admin via Admin Users panel",
    });
  };
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{typeof user.email === 'string' ? user.email : user.email?.address}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-6 text-sm">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-yellow-400 font-semibold">{user.streak || 0}🔥</p>
                    </div>
                    <div className="text-center">
          ...
          <DialogContent className="bg-slate-800 border-slate-700">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent className="bg-slate-700 border-slate-600">
                          ...
                          <SelectItem value="password">Reset Password</SelectItem>
                ...
                {(form.watch('action') === 'balance' || form.watch('action') === 'message' || form.watch('action') === 'admin' || form.watch('action') === 'password') && (
                  <FormField
                    control={form.control}
                    name="value"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-slate-300">
                          {form.watch('action') === 'balance' ? 'Amount' : 
                           form.watch('action') === 'admin' ? 'Admin Status' :
                           form.watch('action') === 'password' ? 'New Password' : 'Message'}
                        </FormLabel>
