...
export default function Profile() {
  ...
  if (!user) return null;
  const currentProfile = { ...user, ...(userProfile || {}) } as any;

  // Prefer server-side profile values (username, firstName) when available.
  const displayName =
    currentProfile.firstName || (user as any)?.name || currentProfile.username || (user?.id ? `user_${user.id.slice(-6)}` : "unknown");
  const usernameToShow = currentProfile.username || "";
              ...
              <div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow overflow-hidden border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="w-full flex items-center px-4 py-4">
                    ...
                    <ProfileQRCode
                      username={currentProfile.username}
                      fullName={currentProfile.firstName || currentProfile.username}
                      profileImageUrl={currentProfile.profileImageUrl}
                      size="sm"
                      trigger={
