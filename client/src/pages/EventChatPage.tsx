...
export default function EventChatPage() {
      ...
      if (data.type === 'event_message' && data.eventId === eventId) {
        ...
        if (data.userId && data.userId !== user?.id) {
          setTypingUsers(prev => {
            ...
            if (data.isTyping) {
              return [...filtered, { userId: data.userId, name: data.username || (data.userId ? `user_${String(data.userId).slice(-6)}` : 'member') }];
            }
            return filtered;
          });
  ...
  useEffect(() => {
    if (user && eventId && sendMessage && isConnected) {
      sendMessage({
        ...
        eventId,
        userId: user.id,
        username: user.firstName || user.username || (user.id ? `user_${user.id.slice(-6)}` : 'member')
      });
    }
  ...
  return (
    <>
      ...
      `}</style>
      <div className="h-[100dvh] min-h-[100dvh] overflow-hidden bg-slate-50 dark:bg-slate-900 flex flex-col md:bg-slate-50 md:dark:bg-slate-900">
      ...
      {/* Chat Messages Area */}
      <div className="flex-1 min-h-0 overflow-y-auto bg-slate-100 dark:bg-slate-800 px-2 md:px-3 py-2 md:py-3">
        {messages.length === 0 ? (
          ...
          <div className="space-y-1 md:space-y-2">
            ...
            const msgUser = message.user ?? { id: message.userId, username: message.user?.username ?? 'unknown', firstName: '', profileImageUrl: undefined, level: 1 };
                ...
                {!isCurrentUser && showAvatar && (
                    ...
                    >
                      <UserAvatar
                        ...
                        profileImageUrl={msgUser.profileImageUrl}
                        size={24}
                        className="w-6 h-6"
                      />
                    </div>
                ...
                <div className={`flex-1 max-w-[75%] ${isCurrentUser ? 'text-right' : ''} ${!showAvatar && !isCurrentUser ? 'ml-8' : ''}`}>
                  ...
                  {message.replyTo && (
                    <div className={`text-xs text-slate-500 dark:text-slate-400 mb-1 ${isCurrentUser ? 'text-right' : ''}`}>
                      ...
                      Replying to {getUserDisplayName({
                        firstName: message.replyTo.user.firstName,
      ...
      {/* Message Input */}
      <div className="shrink-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-2 sticky bottom-0 rounded-t-2xl md:rounded-none mobile-nav-safe-area">
        {!isAuthenticated ? (
          /* Guest User Prompt */
