...
export default function Notifications() {
  ...
  const challengeNotificationTypes = new Set([
    'challenge',
    'challenge_received',
    'challenge_sent',
    'challenge_accepted',
  ...
  const challengeChatNotificationTypes = new Set(['challenge_accepted', 'challenge_active', 'challenge_message']);
  ...
  const getChallengeId = (notification: any): number | null => {
    const data = getNotificationData(notification);
    const rawId = data?.challengeId ?? notification?.challengeId ?? notification?.relatedId;
    const parsed = Number(rawId);
    return Number.isFinite(parsed) ? parsed : null;
  };

  const isChallengeNotification = (notification: any) => challengeNotificationTypes.has(notification?.type);
  const compactActionButtonClass = "h-7 px-2.5 text-[11px] rounded-md";

  const renderChallengeActions = (notification: any) => {
    if (!isChallengeNotification(notification)) return null;

    const challengeId = getChallengeId(notification);
    const notificationData = getNotificationData(notification) || {};
    const scanUrlRaw =
      typeof notificationData.scanUrl === 'string'
        ? notificationData.scanUrl
        : typeof notificationData.explorerUrl === 'string'
          ? notificationData.explorerUrl
          : '';
    const scanUrl = /^https?:\/\/\S+/i.test(scanUrlRaw.trim()) ? scanUrlRaw.trim() : '';
    const canOpenChat = !!challengeId && challengeChatNotificationTypes.has(notification.type);
    ...
    return (
      <div className="mt-2 flex flex-wrap gap-1.5">
        {canOpenChat && (
          <Button
            size="sm"
            className={`${compactActionButtonClass} bg-slate-900 text-white hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200`}
            onClick={() => {
              if (!notification.read) {
                handleMarkAsRead(notification.id);
              }
              window.location.href = `/challenges/${challengeId}/chat`;
            }}
          >
        ...
        <Button
          size="sm"
          className={`${compactActionButtonClass} bg-emerald-600 text-white hover:bg-emerald-700`}
          onClick={() => {
            if (!notification.read) {
              handleMarkAsRead(notification.id);
            }
        ...
        >
          <i className="fas fa-eye mr-1 text-[10px]"></i>
          View Challenges
        </Button>
        {scanUrl && (
          <Button
            size="sm"
            variant="outline"
            className={`${compactActionButtonClass} border-slate-300 text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700`}
            onClick={() => {
              if (!notification.read) {
                handleMarkAsRead(notification.id);
              }
          ...
          >
            <i className="fas fa-up-right-from-square mr-1 text-[10px]"></i>
            Scan
          </Button>
        )}
        {notification.type === 'challenge_received' && (
          <Button
            size="sm"
            variant="outline"
            className={`${compactActionButtonClass} border-red-500 text-red-600 hover:bg-red-50`}
            onClick={() => {
              if (!notification.read) {
                handleMarkAsRead(notification.id);
              }
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex items-center space-x-1 sm:space-x-2 ml-2 sm:ml-4">
                            {!notification.read && (
                              <>
                                <Badge className="bg-primary text-white text-[10px] px-1.5 py-0.5">New</Badge>
                                <Button
                                  size="sm"
                                  variant="ghost"
                        ...
                        {renderChallengeActions(notification)}
                        ...
                        {notification.type === 'new_follower' && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            <Button
                              size="sm"
                              className={`${compactActionButtonClass} bg-purple-600 text-white hover:bg-purple-700`}
                              onClick={() => {
                                if (!notification.read) {
                                  handleMarkAsRead(notification.id);
                                }
                        ...
                        {(notification.type === 'tip_received' || notification.type === 'tip_sent' || notification.type === 'deposit' || notification.type === 'withdrawal') && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            <Button
                              size="sm"
                              className={`${compactActionButtonClass} bg-green-600 text-white hover:bg-green-700`}
                              onClick={() => {
                                if (!notification.read) {
                                  handleMarkAsRead(notification.id);
                                }
                            ...
                            >
                              <i className="fas fa-wallet mr-1 text-[10px]"></i>
                              View Wallet
                            </Button>
                          </div>
                        )}
                        ...
                        {(notification.type === 'event' || notification.type === 'event_starting' || notification.type === 'event_ending' || notification.type === 'funds_locked' || notification.type === 'participant_joined') && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            <Button
                              size="sm"
                              className={`${compactActionButtonClass} bg-blue-600 text-white hover:bg-blue-700`}
                              onClick={() => {
                                if (!notification.read) {
                                  handleMarkAsRead(notification.id);
                                }
                        ...
                        {notification.type === 'daily_login_reminder' && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            <Button
                              size="sm"
                              className={`${compactActionButtonClass} bg-yellow-600 text-white hover:bg-yellow-700`}
                              onClick={() => {
                                if (!notification.read) {
                                  handleMarkAsRead(notification.id);
                                }
                            ...
                            >
                              <i className="fas fa-gift mr-1 text-[10px]"></i>
                              Claim Bonus
                            </Button>
                          </div>
                        )}
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {notification.type === 'friend' && notification.data?.friendRequestId && !notification.data?.friendRequestId && (
                          <div className="mt-2 flex flex-wrap gap-1.5 hidden">
                            <Button
                              size="sm"
                              className={`${compactActionButtonClass} bg-primary text-white hover:bg-primary/90`}
