...
export function NotificationCard({ notification, onMarkAsRead }: NotificationCardProps) {
  ...
  const targetUserId =
    ...
    null;
  const resolvedTargetUser = usePublicUserBasic(targetUserId || undefined, {
    id: targetUserId || undefined,
    firstName: notification?.data?.targetName || notification?.data?.followerName || null,
        ...
        <DialogContent className="sm:max-w-sm max-w-[90vw] max-h-[80vh] overflow-y-auto border-0 shadow-2xl">
          <DialogHeader className="pb-2">
            <DialogTitle className="text-lg font-bold text-center">
              Challenge {getUserDisplayName(resolvedTargetUser, "user")}
            </DialogTitle>
